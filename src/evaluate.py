...
def regenerate_answer(cot, tokenizer, model, case, demo):
    ...
    if cot[0] == ":":
        cot = cot[1:].strip()
    if "Answer:" in cot:
        cot = cot[cot.find("Answer:")+len("Answer:"):].strip()
    if "no," in cot or "No," in cot or "No." in cot or "no." in cot or "NO." in cot:
        cot = "no"
    if "yes," in cot or "Yes," in cot or "Yes." in cot or "yes." in cot or "YES." in cot:
        cot = "yes"
    if "the answer is" in cot:
        return cot
    return " So the answer is " + cot.strip()
...
def main():
    ...
    for line in tqdm(lines):
        ...
        if need_generate:
            pred = regenerate_answer(pred, None, None, case, None)
